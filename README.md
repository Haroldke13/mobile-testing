...
**Status:** does not start — imports `qa.config`, `qa.forms`, `qa.storage`, `qa.testcases`, which the answer never wrote. Bundled anyway so the gap is visible.
...
```bash
./build_cpanel.sh --publish      # rebuild, then stage into ~/Desktop/CPANEL/mobile-testing
~/Desktop/CPANEL_PUSH.sh check mobile-testing
```

Then follow [`cpanel/DEPLOY_STEPS.md`](cpanel/DEPLOY_STEPS.md). Intended
subdomain: **mobile-testing.harold-datascience.co.ke**

## Provenance

`flask/` is the application exactly as the model produced it, copied from `CHATGPT_SOLUTIONS/manual-mobile-app-tester-android-f368cc1f80be/files/` and not edited.
