...
#   ./build_cpanel.sh --publish  rebuild, then copy into ~/Desktop/CPANEL/mobile-testing
set -euo pipefail

HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SRC="$HERE/flask"
DST="$HERE/cpanel"
PUBLISH_DIR="$HOME/Desktop/CPANEL/mobile-testing"
...
if [ "${1:-}" = "--publish" ]; then
  ...
  echo "    now: ~/Desktop/CPANEL_PUSH.sh check mobile-testing"
fi
